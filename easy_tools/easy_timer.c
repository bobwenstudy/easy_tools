...
#define EASY_TIMER_MAX_VALUE          (0xFFFFFFFFU)
#define EASY_TIMER_MAX_VALUE_OVERFLOW (EASY_TIMER_MAX_VALUE >> 1)

static easy_timer_t *easy_timer_root;

static easy_timer_t *_timer_root(void)
{
    return easy_timer_root;
}
...
static uint8_t _timer_past(uint32_t time0, uint32_t time1)
{
    if (time0 <= time1)
    {
        return time1 - time0 < EASY_TIMER_MAX_VALUE_OVERFLOW;
    }

    return time0 - time1 > EASY_TIMER_MAX_VALUE_OVERFLOW;
}
...
{
    easy_timer_t *t = _timer_root();

    if (t)
    {
        easy_tools_api_timer_start(t->expiry_time);
    }
    else
    {
        easy_tools_api_timer_stop();
    }
}
 ...
 */
static int _timer_remove(easy_timer_t *handle)
{
    int is_refresh = 0;
    __easy_disable_isr();
    easy_timer_t *current = _timer_root();
    easy_timer_t *prev = NULL;
 ...
 */
static int _timer_check_in_queue(easy_timer_t *handle)
{
    easy_timer_t *current = _timer_root();
 ...
 * @return 1 if the timer is the new root. 0 if the timer is not the new root.
 */
static int _timer_insert(easy_timer_t *handle)
{
    int is_refresh = 0;
    __easy_disable_isr();
    easy_timer_t *current = _timer_root();
 ...
 */
static int _start_timer(easy_timer_t *handle, uint32_t time)
{
    int is_refresh = 0;
...
static void _timer_expire(void)
{
    int is_refresh = 0;
    easy_timer_t *t;
    while ((t = _timer_root()) != NULL && _timer_past(t->expiry_time, easy_tools_api_timer_get_current()))
    {
        if (t->period)
        {
            is_refresh |= _start_timer(t, _timer_add(easy_tools_api_timer_get_current(), t->period));
            // is_refresh |= _start_timer(t, _timer_add(t->expiry_time, t->period));
        }
        else
        {
...
int easy_timer_start_timer(easy_timer_t *handle, uint32_t ms, uint32_t period)
{
    ...
    if (_start_timer(handle, _timer_add(easy_tools_api_timer_get_current(), ms)))
    {
        _timer_refresh_timeout();
    }
    return 0;
...
void easy_timer_init_timer(easy_timer_t *handle, easy_timer_callback_func callback, void *user_data)
{
    ...
    memset(handle, 0, sizeof(easy_timer_t));
