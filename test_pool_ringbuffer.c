...
static void test_pool_work(void)
{
    ...
    EASY_POOL_DEFINE(test_pool, TEST_BUFFER_SIZE, TEST_USER_DATA_SIZE);

    EASY_POOL_INIT(test_pool, TEST_BUFFER_SIZE, TEST_USER_DATA_SIZE);

    struct test_user_data *ptr_save[TEST_BUFFER_SIZE];

    int total_size = TEST_BUFFER_SIZE;
    ASSERT(EASY_POOL_TOTAL_CNT(&test_pool) == TEST_BUFFER_SIZE);
    ASSERT(EASY_POOL_ITEM_SIZE(&test_pool) == TEST_USER_DATA_SIZE);
    ASSERT(EASY_POOL_SIZE(&test_pool) == total_size);
    ASSERT(EASY_POOL_RESERVE_SIZE(&test_pool) == TEST_BUFFER_SIZE - total_size);
    ASSERT(EASY_POOL_IS_EMPTY(&test_pool) == (total_size == 0));
    ASSERT(EASY_POOL_IS_FULL(&test_pool) == (total_size == TEST_BUFFER_SIZE));

    for (int loop = 0; loop < TEST_BUFFER_SIZE; loop++)
    {
        struct test_user_data *data;
        EASY_POOL_DEQUEUE(&test_pool, data);
    ...
    for (int loop = 0; loop < TEST_BUFFER_SIZE; loop++)
    {
        ...
        EASY_POOL_ENQUEUE(&test_pool, data);
        total_size++;
        ASSERT(EASY_POOL_TOTAL_CNT(&test_pool) == TEST_BUFFER_SIZE);
        ASSERT(EASY_POOL_ITEM_SIZE(&test_pool) == TEST_USER_DATA_SIZE);
        ASSERT(EASY_POOL_SIZE(&test_pool) == total_size);
...
static void test_pool_work_full(void)
{
    ...
    for (int test_cnt = 0; test_cnt < 0x1000; test_cnt++)
    {
        ...
        ASSERT(EASY_POOL_SIZE(&test_pool) == TEST_BUFFER_SIZE - work_cnt);
        ASSERT(EASY_POOL_RESERVE_SIZE(&test_pool) == work_cnt);
        ASSERT(EASY_POOL_IS_EMPTY(&test_pool) == (work_cnt == TEST_BUFFER_SIZE));
        ASSERT(EASY_POOL_IS_FULL(&test_pool) == (work_cnt == 0));
...
static void test_pool_work_odd(void)
{
    ...
    EASY_POOL_DEFINE(test_pool, TEST_BUFFER_SIZE_ODD, TEST_USER_DATA_SIZE_ODD);

    EASY_POOL_INIT(test_pool, TEST_BUFFER_SIZE_ODD, TEST_USER_DATA_SIZE_ODD);

    struct test_user_data_odd *ptr_save[TEST_BUFFER_SIZE_ODD];

    int total_size = TEST_BUFFER_SIZE_ODD;
    ASSERT(EASY_POOL_TOTAL_CNT(&test_pool) == TEST_BUFFER_SIZE_ODD);
    ASSERT(EASY_POOL_ITEM_SIZE(&test_pool) == TEST_USER_DATA_SIZE_ODD);
    ASSERT(EASY_POOL_SIZE(&test_pool) == total_size);
    ASSERT(EASY_POOL_RESERVE_SIZE(&test_pool) == TEST_BUFFER_SIZE_ODD - total_size);
    ASSERT(EASY_POOL_IS_EMPTY(&test_pool) == (total_size == 0));
    ASSERT(EASY_POOL_IS_FULL(&test_pool) == (total_size == TEST_BUFFER_SIZE_ODD));
...
static void test_pool_work_full_odd(void)
{
    ...
    for (int test_cnt = 0; test_cnt < 0x1000; test_cnt++)
    {
        ...
        ASSERT(EASY_POOL_SIZE(&test_pool) == TEST_BUFFER_SIZE_ODD - work_cnt);
        ASSERT(EASY_POOL_RESERVE_SIZE(&test_pool) == work_cnt);
        ASSERT(EASY_POOL_IS_EMPTY(&test_pool) == (work_cnt == TEST_BUFFER_SIZE_ODD));
        ASSERT(EASY_POOL_IS_FULL(&test_pool) == (work_cnt == 0));
